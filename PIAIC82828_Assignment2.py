...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.concatenate((np.stack((arr,arr,arr), axis=1), np.stack((arr,arr,arr), axis=0))).flatten() #write your code here 
...
def function18():
    ...
    x = sum([np.count_nonzero(piaic==i) for i in students]) # Write you code Here
    return x
